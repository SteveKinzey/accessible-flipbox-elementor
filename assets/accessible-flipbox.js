(function ($) {
  ...
  function initAccessibleFlipboxes() {
    $(".elementor-widget-flip-box").each(function (index) {
      ...
      const title =
        $front.find(".elementor-flip-box__layer__title").text().trim() ||
        "Flipbox " + (index + 1);
      ...
      $flipbox.on("click", function (e) {
        ...
        if ($clickedInteractive.length) {
          ...
          const isOnBack =
            $clickedInteractive.closest(".elementor-flip-box__back").length > 0;
          const isOnFront =
            $clickedInteractive.closest(".elementor-flip-box__front").length >
            0;
          ...
          if ((isFlipped && isOnBack) || (!isFlipped && isOnFront)) {
            ...
            const isSubmit = $clickedInteractive.is(
    ...
    console.log(
      "Accessible Flipbox: Initialized " +
        $(".elementor-widget-flip-box").length +
        " flipbox(es)"
    );
  }
})(jQuery);
